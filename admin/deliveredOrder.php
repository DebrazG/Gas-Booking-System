...
<?php 
$st='Delivered';
$query=mysqli_query($conn,"select connection.name as name,connection.address as address,connection.state as state,connection.district as district,connection.city as city,connection.pin as pin,connection.cno as cno,connection.email as email,orders.orderdate as odate,orders.id as id from orders join connection on orders.userid=connection.userid where orders.status='$st'");
$cnt=1;
